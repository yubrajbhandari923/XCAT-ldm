...
class DiffusionUNetResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.up:
            ...
            self.downsample = DiffusionUnetDownsample(
...
class DiffusionModelUNet(nn.Module):
    ...
    def forward(
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            h, res_samples = downsample_block(
                hidden_states=h, temb=emb, context=context[idx]
            )
        ...
        # 5. mid
        h = self.middle_block(hidden_states=h, temb=emb, context=context[-1])
    ...
    def load_old_state_dict(self, old_state_dict: dict, verbose=False) -> None:
        ...
        attention_blocks = [
            k.replace(".attn.to_k.weight", "")
            for k in new_state_dict
            if "attn.to_k.weight" in k
        ]
        for block in attention_blocks:
            new_state_dict[f"{block}.attn.to_q.weight"] = old_state_dict.pop(
                f"{block}.to_q.weight"
            )
            new_state_dict[f"{block}.attn.to_k.weight"] = old_state_dict.pop(
                f"{block}.to_k.weight"
            )
            new_state_dict[f"{block}.attn.to_v.weight"] = old_state_dict.pop(
                f"{block}.to_v.weight"
            )
            new_state_dict[f"{block}.attn.to_q.bias"] = old_state_dict.pop(
                f"{block}.to_q.bias"
            )
            new_state_dict[f"{block}.attn.to_k.bias"] = old_state_dict.pop(
                f"{block}.to_k.bias"
            )
            new_state_dict[f"{block}.attn.to_v.bias"] = old_state_dict.pop(
                f"{block}.to_v.bias"
            )

            # projection
            if (
                f"{block}.attn.out_proj.weight" in new_state_dict
                and f"{block}.attn.out_proj.bias" in new_state_dict
            ):
                new_state_dict[f"{block}.attn.out_proj.weight"] = old_state_dict.pop(
                    f"{block}.proj_attn.weight"
                )
                new_state_dict[f"{block}.attn.out_proj.bias"] = old_state_dict.pop(
                    f"{block}.proj_attn.bias"
                )
        ...
        for block in cross_attention_blocks:
            new_state_dict[f"{block}.out_proj.weight"] = old_state_dict.pop(
                f"{block}.to_out.0.weight"
            )
            new_state_dict[f"{block}.out_proj.bias"] = old_state_dict.pop(
                f"{block}.to_out.0.bias"
            )
