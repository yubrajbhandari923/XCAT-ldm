...
class VolumeSpacingEmbedding(nn.Module):
    def __init__(
        ...
        volume_bins=[
            ...
            1888,
            6000,
        ],  # List of bin edges, e.g., [0, 100, 300, 500, 1000]
...
def build_model(config, rank, train_loader):
    net = DiffusionModelUNet(
        ...
        cross_attention_dim=config.model.params.cross_attention_dim,
    )
    ...
    vol_embed_net = VolumeSpacingEmbedding(
        embed_dim=config.model.volume_embedding.embedding_dim,
        is_discrete=config.model.volume_embedding.is_discrete,
    )

    optimizer = optim.AdamW(
        itertools.chain(net.parameters(), vol_embed_net.parameters()),
        config.optimizer.lr,
    ...
    if rank == 0:
        ...
        num_params += sum(
            p.numel() for p in vol_embed_net.parameters() if p.requires_grad
        )

        logging.info(f"[Rank {rank}] Model parameters: {num_params / 1e6:.2f}M")

    net = auto_model(net)
    vol_embed_net = auto_model(vol_embed_net)
    ...
    def train_step(engine, batchdata):
        ...
        gt_volume = gt_volume / 1000.0  # convert to ml
        vol_context = engine.vol_embed_net(gt_volume, spacing_tensor)  # (B, embed_dim)
        ...
        pred_xstart = engine.network(
            x=noisy_labels, timesteps=timesteps, context=vol_context
        )
        ...
        pred_volume = pred_volume / 1000.0  # convert to ml
        vol_difference = gt_volume - pred_volume
        loss_volume = nn.L1Loss()(pred_volume, gt_volume)
    ...
    trainer.vol_embed_net = vol_embed_net
    ...
    return net, optimizer, lr_scheduler, trainer, vol_embed_net


def get_evaluator(cfg, model, val_loader, vol_embed_net):
    ...
    def _eval_step(engine, batchdata):
        ...
        gt_volume = y_logits.sum(dim=[1, 2, 3, 4]) * voxel_volume  # (B,)
        vol_embed = engine.vol_embed_net(gt_volume, spacing_tensor)  # (B, embed_dim)
        ...
        engine.state.output["Predicted Volume Difference"] = (
    ...
    evaluator.vol_embed_net = vol_embed_net
...
def _distributed_run(rank: int, cfg):
    ...
    model, optimizer, lr_scheduler, trainer, vol_embed_net = build_model(
        cfg, rank, train_loader
    )
    ...
    evaluator = get_evaluator(
        cfg, ema_model if bool(cfg.ema.enable) else model, val_loader, vol_embed_net
    )
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        with idist.Parallel(
            backend=backend, nproc_per_node=nproc, master_port=2225
        ) as parallel:
