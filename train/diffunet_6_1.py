...
from utils.loss import (
    SoftCIDice,
    topo_loss,
    largest_component_dice_loss,
    loss_isoperimetric,
    eikonal_band_match_gt_vec,
    ...
    for organ_idx in conditioning_organs:
        ...
        for data_item in data_list:
            ...
            pred_path = os.path.join(
                organ_predictions_base, f"{filename.replace('.nii.gz', '')}_pred.nii.gz"
            )
            if not os.path.exists(pred_path):
                raise FileNotFoundError(
                    f"Prediction file for {filename} not found in {organ_predictions_base}"
                )
...
def build_full_transform_pipeline(config, train: bool, rank: int):
    ...
    if organ_name != "all_organs":
        ...
        generation_order = list(config.data.generation_order)
        conditioning_organs = get_conditioning_organs(
            generation_order, target_organ_index
        )
        ...
        if rank == 0:
            logging.info("=" * 60)
            logging.info(f"Configuring for all organs generation.")
            logging.info("=" * 60)
    ...
    t = [
        ...
        monai.transforms.CropForegroundd(
            keys=data_keys, source_key=crop_source, margin=5
        ),
        monai.transforms.Resized(
            keys=data_keys, spatial_size=config.data.roi_size, mode="nearest"
        ),
        ...
        MaskToSDFd(
            keys=(
                data_keys + all_organ_keys if organ_name == "all_organs" else data_keys
            ),
        ...
        monai.transforms.ToMetaTensord(
            keys=data_keys + all_organ_keys if organ_name == "all_organs" else data_keys
        ),
        EnsureAllTorchd(print_changes=False),
        monai.transforms.EnsureTyped(
            keys=(
                data_keys + ["spacing_tensor"] + all_organ_keys
                if organ_name == "all_organs"
                else data_keys + ["spacing_tensor"]
            ),
            track_meta=True,
        ),
    ]
    ...
    if config.training.inference_mode:
        pred_keys = [
            f"pred_{ORGAN_NAMES.get(idx, str(idx))}" for idx in conditioning_organs
        ]
        if len(pred_keys) != 0:
            t.extend(
                [
                    ...
                    monai.transforms.Orientationd(
                        keys=pred_keys, axcodes=config.data.orientation
                    ),
...
def get_dataloaders(config, aim_logger, rank: int):
    ...
    if config.training.inference_mode:
        if rank == 0:
            logging.info(
                "Inference mode: Adding conditioning predictions to validation files"
            )
    ...
    if organ_name != "all_organs":
        ...
        base_cache_root = os.path.join(config.data.cache_dir, f"{organ_name}")
        if rank == 0:
            logging.info(f"Cache root: {base_cache_root} | All organs")
    os.makedirs(base_cache_root, exist_ok=True)

    # ---------------- transforms ----------------
    train_transform = build_full_transform_pipeline(config, train=True, rank=rank)
    val_transform = build_full_transform_pipeline(config, train=False, rank=rank)
...
def prepare_batch(batch, device=None, non_blocking=True):
    ...
    if "body_filled_mask" in batch:
        body_filled_mask = batch["body_filled_mask"].to(
...
def build_model(config, rank, train_loader):
    net = DiffusionModelUNet(
        ...
        + config.model.params.out_channels,  # add image SDF as extra channel
    ...
    def train_step(engine, batchdata):
        ...
        noisy_labels = torch.cat(
            [noisy_labels, images], dim=1
        )  # concat image SDF as extra channel
        ...
        image_logits = sdf_to_mask(
            images[:, 0:1, ...]
        )  # assuming first channel is the mask SDF
        loss_reverse_dice = 1 - engine.reverse_dice_loss(pred_prob, image_logits)

        loss = (loss_l1 + loss_bce) + loss_ci + loss_reverse_dice
        loss = loss / accum
    ...
    if bool(config.model.adverserial_train.enabled):
        ...
        trainer.adv_lambda = float(
            config.model.adverserial_train.adverserial_loss_weight
        )
...
def get_evaluator(cfg, model, val_loader):
    ...
    post = transforms.Compose(
        [
            ...
            (
                ...
                if cfg.task == "colon"
                else transforms.Identityd(keys=[Keys.PRED])
            ),
    ...
    def cosine_ramp(t, T, lam_max):
        # weak early, stronger late
        s = (t+1) / T
        return lam_max * (1.0 - math.cos(0.5*math.pi*s)**2)

    def clip_grad(g, gmax):
        g_norm = g.flatten(1).norm(p=2, dim=1, keepdim=True) + 1e-12
        scale = (g_norm / gmax).clamp(min=1.0)
        return g / scale.view(-1, *([1]*(g.ndim-1)))
    ...
    def _eval_step(engine, batchdata):
        ...
        for t, next_t in iter(zip(engine.scheduler.timesteps, all_next_timesteps)):
            model_input = torch.cat(
                [pred, image], dim=1
            )  # concat image SDF as extra channel # modify to include classifier guidance later

            model_output = engine.network(
                x=model_input, timesteps=torch.Tensor((t,)).to(image.device)
            )
            # ===== training-free guidance block (ensure grads are enabled) =====
            with torch.enable_grad():
                x0 = model_output
                x0 = x0.requires_grad_(True)   # DO NOT detach

                # Soft, differentiable occupancy from SDF (neg-inside)
                scale = 10.0   # your previous scaling
                beta  = 0.05   # ~2–10% of a voxel; tune
                soft_occ = torch.sigmoid(-(x0 * scale) / beta)   # float, differentiable
                vol = soft_occ.sum(dim=(1, 2, 3, 4))

                # Use voxel volume as a constant (keep it OUT of the graph)
                voxel_vol = float(spacing_tensor.prod().item())
                vol = vol * voxel_vol                       # [B], requires_grad=True

                target_volume = torch.full_like(vol, 55154908.0)
                volume_loss = torch.nn.functional.mse_loss(vol, target_volume)

                # Get gradient wrt x0
                g = torch.autograd.grad(volume_loss, x0, retain_graph=False, create_graph=False)[0]

            # with torch.enable_grad():
            #     model_output = model_output.requires_grad_(True)
            #     logging.info(f"Model Output Shape: {model_output.shape}, requires_grad: {model_output.requires_grad}")
            #     model_output_mask = sdf_to_mask(model_output * 10.0)
            #     logging.info(f"Model Output Mask Shape: {model_output_mask.shape}, requires_grad: {model_output_mask.requires_grad}")
            #     model_output_volume = model_output_mask.sum(dim=[1, 2, 3]).float() * spacing_tensor.prod()
            #     logging.info(f"Model Output Volume Shape: {model_output_volume.shape}, requires_grad: {model_output_volume.requires_grad}")
            #     target_volume = torch.tensor(140000.0, device=model_output_volume.device)
            #     volume_loss = nn.MSELoss()(model_output_volume, torch.full_like(model_output_volume, target_volume))
            #     logging.info(f"Volume Loss: {volume_loss.item()}")

            #     g = torch.autograd.grad(volume_loss, model_output)[0]
            #     # lam_t = cosine_ramp(k, T, lam_max)
            #     # g = clip_grad(g, gmax)

            model_output = (model_output - model_output * g * 5e-7).detach()  # apply guidance, detach to avoid messing up the graph
            ...
            if cfg != 1.0:
                ...
                uncond_output = engine.network(
                    x=model_input_uncond, timesteps=torch.Tensor((t,)).to(image.device)
                )
                model_output = uncond_output + cfg * (model_output - uncond_output)

            pred, _ = engine.scheduler.step(model_output, t, pred)

        engine.state.output = {
            Keys.IMAGE: sdf_to_mask(image),
            Keys.LABEL: sdf_to_mask(masks),
        }
        ...
        scale = 10.0   # your previous scaling
        beta  = 0.05   # ~2–10% of a voxel; tune
        soft_occ = torch.sigmoid(-(masks * scale) / beta)   # float, differentiable
        mask_vol = soft_occ.sum(dim=(1, 2, 3, 4))

        soft_occ_pred = torch.sigmoid(-(pred * scale) / beta)   # float, differentiable
        pred_vol = soft_occ_pred.sum(dim=(1, 2, 3, 4))

        pred = sdf_to_mask(pred * 10.0)
        engine.state.output[Keys.PRED] = pred
        logging.info(f"mask volume: {mask_vol * spacing_tensor.prod()} \n Prediction Volume: {pred_vol * spacing_tensor.prod()}")
...
if __name__ == "__main__":
    ...
    def main(cfg: DictConfig):
        ...
        with idist.Parallel(
            backend=backend, nproc_per_node=nproc, master_port=2222
        ) as parallel:
